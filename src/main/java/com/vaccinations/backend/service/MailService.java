...
import com.vaccinations.backend.model.Mail;
...
public class MailService {
    ...
    public Mail createRegistrationConfirmMail(Vaccination vaccination, Patient patient) {
        Mail mail = createRegistrationMailTemplate(patient.getEmail());
        Map<String, Object> model = new HashMap<>();
        log.info(String.valueOf(patient.getPesel().charAt(9) - '0'));
        model.put("sex", (patient.getPesel().charAt(9) - '0') % 2 == 0 ? "FEMALE" : "MALE");
        model.put("date", vaccination.getDate());
        model.put("time", vaccination.getTime());
        model.put("address", vaccination.getAddress());
        mail.setModel(model);
